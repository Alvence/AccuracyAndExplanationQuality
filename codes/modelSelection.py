...
root_path = "/datadrive/dataset/Kahikatea/"
data_dir = root_path + "data/"
stats_path = "/datadrive/stats/Kahikatea/"
#data_dir = "D:/CUB_200_2011/CUB_200_2011/data/"
...
#save_to_path = "D:/CUB_200_2011/CUB_200_2011/models/"
...
def eval_expl_recall(expl, true_expl, threshold=0.01):
    ...
    return tp/np.nansum(t)
    
from os import path
def evaluate_expl(model_name, model, dataloaders, explainer='guidedGradCAM', saved_model_name = None):
    aucs = {'train':[], 'val':[], 'test':[]}
    ...
    for phase in ['val','test']:
        ...
        for ind in idx:
            ...
            mask_path = img_path.replace('/data/ve','/expl/ve')
            ...
            try:
                ex = viz._normalize_image_attr(tmp, 'absolute_value', 2)
            except:
                print('normalize with zero scale!')
                continue
            ex = np.abs(ex)
            ...
            #mask.save(data_dir+'/temp/mask/'+y.split('/')[1]+'.jpg')
            ...
            mask = 1 - mask
            mask[mask > 0] = 1
            #plt.imsave(data_dir+'/temp/mask/'+y.split('/')[1]+'.jpg', mask)
            #plt.imsave(data_dir+'/temp/ex/'+y.split('/')[1]+'.jpg', ex)
            ...
            if np.sum(mask) == 0:
                mask[0][0] = 1
            if np.sum(mask) >= 224*224:
                mask[0][0] = 0
                
            if np.sum(ex) == 0:
                ex[0][0] = 1
            if np.sum(ex) >= 224*224:
                ex[0][0] = 0
...
def run(model_name, random_state, explainer, train_ratio, val_ratio):
    ...
    df.to_csv(stats_path + model_name + "_"+explainer+"_"+str(random_state)+'_'+str(train_ratio)+'_'+str(val_ratio)+".csv", index=False)
...
if __name__ == '__main__':
    ...
    for model_name in  model_names:
        for random_state in range(num_runs):
            print(explainer+'-'+model_name + '-' + str(random_state))
